...
public class MCP5Remapper extends BaseRemapper {

    public MCP5Remapper(final Map<String, byte[]> entries, final File mappings) {
        super(entries, mappings);
    }

    @Override
    protected MappingsLoader initLoader(File mappings) throws Exception {
        File srgConfig = FileUtils.create(mappings, "conf", "joined.srg");
